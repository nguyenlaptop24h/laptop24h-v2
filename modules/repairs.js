...
<div class="rfm-r rfm-r1" style="margin-bottom:2px"><div class="rfm-f" style="padding:0"><button type="button" id="f-rd-toggle" style="width:100%;text-align:left;background:#f1f5f9;border:1px solid #cbd5e1;border-radius:8px;padding:8px 12px;font-size:13px;font-weight:700;color:#334155;cursor:pointer">▸ Chi tiết sửa chữa (nội dung, phụ kiện, linh kiện, dịch vụ)</button></div></div>
<div id="f-rdetails">
...
<div class="rfm-r rfm-r2"><div class="rfm-f"><label>TRẠNG THÁI</label><select id="f-status">${STATUS_LIST.map(s => '<option ' + ((record?.status||'Tiếp nhận')===s ? 'selected' : '') + '>' + s + '</option>').join('')}</select></div><div class="rfm-f"><label>GHI CHÚ NỘI BỘ</label><textarea id="f-internalNote" placeholder="Chỉ nhân viên thấy...">${record?.internalNote||''}</textarea></div></div>
...
<div class="rfm-foot"><button class="rfm-cancbtn" id="f-cancel">Hủy</button><button id="f-print" style="padding:9px 22px;border:1.5px solid #0ea5e9;background:#0ea5e9;color:#fff;border-radius:8px;cursor:pointer;font-size:14px;font-weight:500">🖨 In phiếu</button><button class="rfm-savbtn" id="f-save">💾 Lưu phiếu</button></div>
  ...
  // Dữ liệu cũ: suy ra "vốn LK mua ngoài" = vốn đã lưu - vốn linh kiện từ kho
  ...
  // ── Thu gọn "Chi tiết sửa chữa": mở sẵn khi SỬA phiếu, thu gọn khi THÊM phiếu mới ──
  (function(){
    var _rd = formWrap.querySelector('#f-rdetails');
    var _rdT = formWrap.querySelector('#f-rd-toggle');
    if(!_rd || !_rdT) return;
    var _open = !!record;
    function _apply(){ _rd.style.display = _open ? 'block' : 'none'; _rdT.innerHTML = (_open?'▾':'▸') + ' Chi tiết sửa chữa (nội dung, phụ kiện, linh kiện, dịch vụ)'; }
    _apply();
    _rdT.addEventListener('click', function(){ _open = !_open; _apply(); });
  })();
