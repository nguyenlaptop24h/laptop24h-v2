...
<div class="rfm-r rfm-r1"><div class="rfm-f"><label>T�NH TRẠNG KHI NHẬN (M� TẢ LỖI)</label><textarea id="f-initialCondition" placeholder="Kh�ng l�n nguồn, m�n h�nh trắng, b�n ph�m liệt...">${record?.initialCondition||''}</textarea></div></div>
<div class="rfm-r rfm-r1"><div class="rfm-f"><label>PHỤ KIỆN K�M THEO</label><input id="f-accessories" type="text" placeholder="Sạc, t�i, chuột..." value="${record?.accessories||''}"></div></div>
<div class="rfm-r rfm-r3"><div class="rfm-f"><label>NG�Y NHẬN *</label><input id="f-receivedDate" type="date" value="${record?.receivedDate||new Date().toISOString().slice(0,10)}"></div><div class="rfm-f"><label>CHI PH� DỰ KIẾN (Đ)</label><input id="f-cost" type="text" data-fmt="number" value="${String(record?.cost||0).replace(/\B(?=(\d{3})+(?!\d))/g,'.')}"></div><div class="rfm-f"><label>TIỀN CỌC (Đ)</label><input id="f-deposit" type="text" data-fmt="number" value="${String(record?.deposit||0).replace(/\B(?=(\d{3})+(?!\d))/g,'.')}"></div></div>
...
<div class="rfm-r rfm-r2"><div class="rfm-f"><label>BẢO H�NH SỬA CHỮA</label><select id="f-warranty"><option value="3 th�ng" ${(record?.warranty||'3 th�ng')==='3 th�ng'?'selected':''}>3 th�ng</option><option value="6 th�ng" ${record?.warranty==='6 th�ng'?'selected':''}>6 th�ng</option><option value="1 năm" ${record?.warranty==='1 năm'?'selected':''}>1 năm</option><option value="Kh�ng bảo h�nh" ${record?.warranty==='Kh�ng bảo h�nh'?'selected':''}>Kh�ng bảo h�nh</option></select></div><div class="rfm-f"><label>KỸ THUẬT VI�N</label><input id="f-techName" type="text" placeholder="T�n KTV..." value="${record?.techName||''}"></div></div>
<div class="rfm-r rfm-r1"><div class="rfm-f"><label>GHI CH� NỘI BỘ</label><textarea id="f-internalNote" placeholder="Chỉ nh�n vi�n thấy...">${record?.internalNote||''}</textarea></div></div>
