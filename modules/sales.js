...
export async function mount(container) {
  ...
  // Load sáº£n pháº©m tá»« Firestore (collection 'products' giá»ng inventory.js)
...
/* FORM GRID â 3 cols for top info */
...
<div class="sl-wrap">
  <div class="sl-toolbar">
    <button class="sl-btn sl-btn-primary" id="sl-add-btn">ï¼ BÃ¡n hÃ ng</button>
    <button class="sl-btn sl-btn-trash" id="sl-trash-btn">ð ThÃ¹ng rÃ¡c</button>
    <div class="sl-search">
      <span style="font-size:13px;color:#aaa">ð</span>
      <input id="sl-search-inp" placeholder="TÃ¬m khÃ¡ch hÃ ng, sáº£n pháº©m...">
    </div>
    <div class="sl-filters" id="sl-filters">
      <span class="sl-f active" data-mode="day">HÃ´m nay</span>
      <span class="sl-f" data-mode="week">Tuáº§n</span>
      <span class="sl-f" data-mode="month">ThÃ¡ng</span>
      <span class="sl-f" data-mode="all">Táº¥t cáº£</span>
    </div>
    <div class="sl-spacer"></div>
    <button class="sl-btn sl-btn-tpl" id="sl-tpl-btn" title="CÃ i Äáº·t máº«u hÃ³a ÄÆ¡n">ð¨ Máº«u HÄ</button>
    <div class="sl-quickstats">
      ÄÆ¡n: <b id="sl-qs-count">0</b> &nbsp;Â·&nbsp; Doanh thu: <b id="sl-qs-rev">0Ä</b>
    </div>
  </div>
  ...
  <div class="sl-tpl-overlay" id="sl-tpl-overlay" style="display:none">
    <div class="sl-tpl-modal">
      <div class="sl-tpl-header">
        <h3>ð¨ CÃ i Äáº·t máº«u hÃ³a ÄÆ¡n</h3>
        <button class="sl-close-btn" id="sl-tpl-close">â</button>
      </div>
      <div class="sl-tpl-body">
        <div class="sl-tpl-grid">
          ...
          <div class="sl-field">
            ...
            <input id="tpl-address" placeholder="123 ÄÆ°á»ng ABC, Quáº­n 1, TP.HCM">
          </div>
          <div class="sl-field">
            <label>Sá» Äiá»n thoáº¡i cá»­a hÃ ng</label>
            <input id="tpl-phone" placeholder="0901 234 567">
          </div>
          <div class="sl-field">
            <label>TiÃªu Äá» hÃ³a ÄÆ¡n</label>
            <input id="tpl-title" placeholder="HÃA ÄÆ N BÃN HÃNG">
          </div>
          <div class="sl-field">
            <label>Ghi chÃº cuá»i trang</label>
            <textarea id="tpl-footer" rows="2" style="resize:vertical;padding:7px 9px;border:1px solid #ddd;border-radius:6px;font-size:12.5px;font-family:inherit;outline:none" placeholder="Cáº£m Æ¡n quÃ½ khÃ¡ch ÄÃ£ mua hÃ ng! ð"></textarea>
          </div>
        </div>
      </div>
      <div class="sl-tpl-footer">
        <button class="sl-btn" id="tpl-cancel">Huá»·</button>
        <button class="sl-btn sl-btn-primary" id="tpl-save">ð¾ LÆ°u máº«u</button>
      </div>
    </div>
  </div>
</div>`;
  ...
  container.querySelector('#tpl-save').onclick = () => {
    ...
    toast('ÄÃ£ lÆ°u máº«u hÃ³a ÄÆ¡n â');
  };
  ...
  function renderTable(filtered) {
    ...
    if (!total) {
      const msg = filterMode === 'trash' ? 'ThÃ¹ng rÃ¡c trá»ng' : 'ChÆ°a cÃ³ ÄÆ¡n nÃ o';
      const icon = filterMode === 'trash' ? 'ð' : 'ð';
      wrap.innerHTML = `<div class="sl-empty"><div style="font-size:2.5rem">${icon}</div><div>${msg}</div></div>`;
      renderPagination(0, 0); return;
    }

    const STATUS = {
      done:    { label: 'HoÃ n thÃ nh', cls: 'badge-done' },
      pending: { label: 'Chá» TT',     cls: 'badge-pending' },
      cancel:  { label: 'ÄÃ£ huá»·',     cls: 'badge-cancel' },
    };
    ...
    const rows = page.map((s, i) => {
      ...
      const items   = s.items || [];
      const itemStr = items.length === 0 ? 'â'
        : items.length === 1 ? (items[0].name || 'â')
        : `${items[0].name || ''} +${items.length - 1}`;
      const st  = STATUS[s.status || 'done'] || STATUS['done'];
      const pay = s.payMethod === 'transfer' ? 'ð¦ CK' : 'ðµ TM';
      ...
      if (filterMode === 'trash') {
        return `<tr>
          ...
          <td>${pay}</td>
          <td><span class="sl-badge badge-trash">ÄÃ£ xoÃ¡</span></td>
          <td>${s.date || ''}</td>
          <td><button class="sl-action-btn sl-restore-btn" data-key="${s._key}">â© KhÃ´i phá»¥c</button></td>
        </tr>`;
      }
      ...
      return `<tr>
        ...
        <td>${code}</td>
        <td>
          <div class="sl-customer">${esc(s.customer || 'â')}</div>
        ...
        <td>
          <div class="sl-actions">
            <button class="sl-action-btn sl-edit-btn"  data-key="${s._key}">â</button>
            <button class="sl-action-btn sl-print-btn" data-key="${s._key}">ð¨</button>
            <button class="sl-action-btn sl-del sl-del-btn" data-key="${s._key}">ð</button>
          </div>
        </td>
      </tr>`;
    });
  ...
  function renderPagination(total, totalPages) {
    ...
    let html = `<span style="margin-right:6px">Hiá»n thá» ${from}â${to} / ${total} ÄÆ¡n</span>`;
    html += `<button class="sl-pg-btn" ${currentPage === 1 ? 'disabled' : ''} data-pg="${currentPage - 1}">â¹</button>`;
    const maxP = Math.min(totalPages, 7);
    for (let p = 1; p <= maxP; p++)
      html += `<button class="sl-pg-btn ${p === currentPage ? 'active' : ''}" data-pg="${p}">${p}</button>`;
    if (totalPages > maxP) html += `<span style="padding:0 4px">â¦${totalPages}</span>`;
    html += `<button class="sl-pg-btn" ${currentPage === totalPages ? 'disabled' : ''} data-pg="${currentPage + 1}">âº</button>`;
    pg.innerHTML = html;
  ...
  function renderSidebar() {
    ...
    <div class="sl-sb-section">
      <div class="sl-sb-title">ThÃ¡ng ${todayStr.slice(5, 7)}</div>
      <div class="sl-stat-card">
        <div class="sl-stat-val" style="font-size:1.05rem">${formatVND(monthRev)}</div>
        <div class="sl-stat-lbl">${monthList.length} ÄÆ¡n</div>
      </div>
    </div>
    <div class="sl-sb-section">
      <div class="sl-sb-title">7 ngÃ y gáº§n ÄÃ¢y</div>
      <div class="sl-chart">
        ${days7.map(d => {
    ...
    <div class="sl-sb-section">
      <div class="sl-sb-title">Thanh toÃ¡n</div>
      <div class="sl-pay-row"><span>ðµ Tiá»n máº·t</span><b>${cashPct}%</b></div>
      ...
      <div class="sl-pay-row"><span>ð¦ Chuyá»n khoáº£n</span><b>${tranPct}%</b></div>
    </div>`;
  }
  ...
  function openForm(key) {
    ...
    <div class="sl-modal-header">
      <h3>${ex ? 'âï¸ Sá»­a ÄÆ¡n bÃ¡n hÃ ng' : 'â ThÃªm ÄÆ¡n bÃ¡n hÃ ng'}</h3>
      <button class="sl-close-btn" id="sl-close">â</button>
    </div>
    <div class="sl-modal-body">
      ...
      <div class="sl-form-grid">
        <div class="sl-field span2">
          ...
          <input id="sf-customer" placeholder="Nháº­p tÃªn khÃ¡ch hÃ ng...">
        </div>
        <div class="sl-field">
        ...
        <div class="sl-field">
          <label>NgÃ y bÃ¡n</label>
          <input id="sf-date" type="date">
        </div>
        <div class="sl-field">
          <label>Thanh toÃ¡n</label>
          <select id="sf-pay">
            <option value="cash">ðµ Tiá»n máº·t</option>
            <option value="transfer">ð¦ Chuyá»n khoáº£n</option>
          </select>
        </div>
        <div class="sl-field">
          <label>Tráº¡ng thÃ¡i</label>
          <select id="sf-status">
            <option value="done">â HoÃ n thÃ nh</option>
            <option value="pending">â³ Chá» thanh toÃ¡n</option>
            <option value="cancel">â ÄÃ£ huá»·</option>
          </select>
        </div>
        <div class="sl-field span2">
          <label>Ghi chÃº</label>
          <input id="sf-note" placeholder="Ghi chÃº thÃªm...">
        </div>
      </div>

      <div class="sl-items-header">
        <div class="sl-items-label">ð¦ Sáº£n pháº©m</div>
      </div>
      <div style="display:flex;gap:5px;padding:4px 9px;font-size:10px;font-weight:700;color:#aaa;text-transform:uppercase;letter-spacing:.3px">
        <div style="flex:1">TÃªn sáº£n pháº©m</div>
        <div style="width:56px;text-align:center">SL</div>
        <div style="width:110px;text-align:right">ÄÆ¡n giÃ¡</div>
        <div style="width:80px;text-align:right">Giáº£m</div>
        <div style="width:120px;text-align:center">Háº¿t BH</div>
        <div style="width:90px;text-align:right">ThÃ nh tiá»n</div>
        <div style="width:22px"></div>
      </div>
      <div id="sf-rows"></div>
      <button id="sf-add-row" class="sl-add-row-btn">ï¼ ThÃªm sáº£n pháº©m</button>

      <div class="sl-totals">
        <div class="sl-total-row">
          <span>Táº¡m tÃ­nh:</span>
          <b id="sf-subtotal" style="font-size:13px">0Ä</b>
        </div>
        <div class="sl-total-row">
          <label for="sf-extra-disc" style="cursor:pointer;color:#555">Giáº£m thÃªm:</label>
          <input id="sf-extra-disc" type="number" min="0" value="0" placeholder="0">
        </div>
        <div class="sl-total-row sl-total-final">
          <span style="font-weight:700;font-size:13px">Tá»ng cá»ng:</span>
          <b id="sf-total" style="color:#1d4ed8;font-size:1.1rem">0Ä</b>
        </div>
        <div class="sl-total-row">
          <label for="sf-paid" style="cursor:pointer;color:#555">ÄÃ£ tráº£:</label>
          <input id="sf-paid" type="number" min="0" value="0" placeholder="0">
        </div>
      </div>
    </div>
    <div class="sl-modal-footer">
      <button class="sl-btn" id="sf-cancel">Huá»·</button>
      ${ex ? `<button class="sl-btn sl-btn-trash" id="sf-del-modal">ð XoÃ¡</button>` : ''}
      ${ex ? `<button class="sl-btn sl-btn-tpl" id="sf-print-modal">ð¨ In HÄ</button>` : ''}
      <button class="sl-btn sl-btn-primary" id="sf-save">ð¾ LÆ°u ÄÆ¡n</button>
    </div>`;

    // ââ Set values programmatically (trÃ¡nh bug value rá»ng vá»i innerHTML) ââ
  ...
  function addItemRow(wrap, data) {
    ...
    row.innerHTML = `
      <input class="sf-name"  placeholder="TÃªn sáº£n pháº©m..." autocomplete="off" style="flex:1;min-width:0">
      <input class="sf-qty"   type="number" min="1"  title="Sá» lÆ°á»£ng">
      <input class="sf-price" type="number" min="0"  title="ÄÆ¡n giÃ¡" style="width:110px">
      <input class="sf-disc"    type="number" min="0"  title="Giáº£m giÃ¡" style="width:80px">
      <input class="sf-bh-date" type="date"            title="NgÃ y háº¿t báº£o hÃ nh">
      <span class="sf-line-total">0Ä</span>
      <button class="sf-remove-btn" type="button" title="XoÃ¡ dÃ²ng">â</button>`;
  ...
  async function saveForm() {
    ...
    if (!items.length) { toast('Vui lÃ²ng thÃªm Ã­t nháº¥t 1 sáº£n pháº©m'); return; }
    ...
    saveBtn.disabled = true; saveBtn.textContent = 'Äang lÆ°u...';

    try {
      if (editKey) {
        await updateItem(COLLECTION, editKey, data);
        toast('Cáº­p nháº­t thÃ nh cÃ´ng â');
        logToSheet({ ...data, key: editKey }, 'update');
      } else {
        const ref = await addItem(COLLECTION, data);
        toast('LÆ°u ÄÆ¡n thÃ nh cÃ´ng â');
        logToSheet({ ...data, key: ref?.key || '' }, 'add');
      }
      // Trừ tồn kho (chỉ khi tạo đơn mới)
      if (!editKey) {
        for (const it of items) {
          if (!it.invkey) continue;
          const prod = invItems.find(p => p['_key'] === it.invkey);
          if (prod) {
            const newStock = Math.max(0, (Number(prod.stock) || 0) - (Number(it.qty) || 0));
            await updateItem('products', it.invkey, { stock: newStock });
          }
        }
      }
      ...
      saveBtn.disabled = false; saveBtn.textContent = 'ð¾ LÆ°u ÄÆ¡n';
    }
  }

  // ââââââââââââââââââââââââââââââââââââââââââââââ
  //  PRINT INVOICE (Phiáº¿u Báº£o HÃ nh â thiáº¿t káº¿ má»i)
  // ââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  function printInvoice(key) {
    ...
    if (!s) { toast('KhÃ´ng tÃ¬m tháº¥y ÄÆ¡n hÃ ng'); return; }
    ...
    const addr       = tpl.address ? esc(tpl.address) : '123 ÄÆ°á»ng ABC, TP.HCM';
    const phone      = tpl.phone   ? esc(tpl.phone)   : '0909 123 456';
    const invTitle   = esc(tpl.title   || 'PHIáº¾U Báº¢O HÃNH');
    const footerText = esc(tpl.footer  || 'Cáº£m Æ¡n quÃ½ khÃ¡ch ÄÃ£ tin tÆ°á»ng mua sáº¯m! ð');
    ...
    const itemRows = (s.items || []).map((it, i) => {
      ...
      const bhChip = it.bhDate
        ...
        : `<span style="color:#bbb;font-size:12px">â</span>`;
      return `<tr>
        <td style="text-align:center;color:#888">${i + 1}</td>
        <td>${esc(it.name || '')}${it.discount ? `<br><span style="font-size:11px;color:#888">Giáº£m: ${fmt(it.discount)}Ä</span>` : ''}</td>
        <td style="text-align:center">${it.qty || 1}</td>
        <td style="text-align:right">${fmt(it.price || 0)}</td>
        <td style="text-align:right;font-weight:600">${fmt(line)}Ä</td>
...
<div class="page">
  ...
  <div class="header">
    <div>
      ...
      <div class="shop-sub">ð ${addr}<br>ð ${phone}</div>
    </div>
  ...
  <div class="ribbon">
    <span>ð NgÃ y bÃ¡n: ${dateStr}</span>
    <span>${s.staff ? 'ð¤ NhÃ¢n viÃªn: ' + esc(s.staff) : ''}</span>
  </div>
  <div class="body">
    <div class="cust-box">
      <div class="fi"><span class="k">KhÃ¡ch hÃ ng: </span>${esc(s.customer || 'KhÃ¡ch láº»')}</div>
      ${s.phone ? `<div class="fi"><span class="k">Äiá»n thoáº¡i: </span>${esc(s.phone)}</div>` : ''}
      <div class="fi"><span class="k">Thanh toÃ¡n: </span>${s.payMethod === 'transfer' ? 'ð¦ Chuyá»n khoáº£n' : 'ðµ Tiá»n máº·t'}</div>
      ${s.note ? `<div class="fi"><span class="k">Ghi chÃº: </span>${esc(s.note)}</div>` : ''}
    </div>
    <table>
      <thead>
        <tr>
          ...
          <th style="width:90px" class="tr">ÄÆ¡n giÃ¡</th>
          <th style="width:95px" class="tr">ThÃ nh tiá»n</th>
          <th style="width:112px" class="tc">BH Äáº¿n ngÃ y</th>
        </tr>
      </thead>
      <tbody>${itemRows}</tbody>
    </table>
    <div class="totals-wrap"><div class="totals-box">
      ${s.extraDiscount ? `<div class="t-row disc"><span class="k">Giáº£m thÃªm:</span><span class="v">-${fmt(s.extraDiscount)}Ä</span></div>` : ''}
      <div class="t-row grand"><span class="k">Tá»NG THANH TOÃN</span><span class="v">${fmt(s.total || 0)}Ä</span></div>
      <div class="t-row"><span class="k">ÄÃ£ tráº£:</span><span class="v">${fmt(s.paid || 0)}Ä</span></div>
      ${remaining > 0 ? `<div class="t-row owe"><span class="k">CÃ²n láº¡i:</span><span class="v">${fmt(remaining)}Ä</span></div>` : ''}
    </div></div>
    <div class="wn-box">
      <div class="wn-head">ð ÄIá»U KHOáº¢N Báº¢O HÃNH</div>
      <div class="wn-body"><ul>
        <li>Báº£o hÃ nh Ã¡p dá»¥ng cho lá»i ká»¹ thuáº­t phÃ¡t sinh trong Äiá»u kiá»n sá»­ dá»¥ng bÃ¬nh thÆ°á»ng.</li>
        <li>KhÃ´ng Ã¡p dá»¥ng vá»i cÃ¡c trÆ°á»ng há»£p: vá»¡, vÃ o nÆ°á»c, chÃ¡y ná», tá»± Ã½ thÃ¡o láº¯p.</li>
        <li>Vui lÃ²ng xuáº¥t trÃ¬nh phiáº¿u nÃ y khi yÃªu cáº§u báº£o hÃ nh.</li>
        <li>Hotline há» trá»£: <strong>${phone}</strong> (8hâ21h, Thá»© 2 â Chá»§ Nháº­t).</li>
      </ul></div>
    </div>
  </div>
  <div class="footer-band">
    <strong>${shopName} â ${phone}</strong>
    <span>${footerText}</span>
  </div>
  <div class="bottom-stripe"></div>
  <div class="btn-bar">
    <button class="btn-p" onclick="window.print()">ð¨ In phiáº¿u báº£o hÃ nh</button>
    <button class="btn-c" onclick="window.close()">â ÄÃ³ng</button>
  </div>
</div>
</body></html>`;
    ...
    if (!win) { toast('â  Vui lÃ²ng cho phÃ©p popup Äá» in phiáº¿u'); URL.revokeObjectURL(url); return; }
  ...
  async function softDelete(key) {
    if (!confirm('Chuyá»n ÄÆ¡n nÃ y vÃ o thÃ¹ng rÃ¡c?')) return;
    try {
      await updateItem(COLLECTION, key, { deletedAt: Date.now() });
      toast('ÄÃ£ chuyá»n vÃ o thÃ¹ng rÃ¡c');
    } catch(e) { toast('Lá»i: ' + e.message); }
  }

  async function restoreItem(key) {
    try {
      await updateItem(COLLECTION, key, { deletedAt: null });
      toast('ÄÃ£ khÃ´i phá»¥c â');
    } catch(e) { toast('Lá»i: ' + e.message); }
  }
