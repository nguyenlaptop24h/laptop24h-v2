...
import { initRouter } from './core/router.js?v=173';
...
// ── Đồng bộ icon: thay emoji trong giao diện bằng icon line Lucide (currentColor) ──
const _svg = i => '<svg class="e-ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' + i + '</svg>';
const EMAP = {
...
const _EKEYS = Object.keys(EMAP);
const _ERE = new RegExp('(?:' + _EKEYS.join('|') + ')\\uFE0F?', 'gu');
const _ESKIP = new Set(['OPTION', 'SELECT', 'TEXTAREA', 'SCRIPT', 'STYLE', 'TITLE', 'OPTGROUP', 'INPUT', 'NOSCRIPT']);
function _iconize(node) {
  if (node.nodeType === 1) {
    if (node.namespaceURI === 'http://www.w3.org/2000/svg') return;
    if (_ESKIP.has(node.tagName)) return;
    let c = node.firstChild;
    while (c) { const nx = c.nextSibling; _iconize(c); c = nx; }
    return;
  }
  if (node.nodeType !== 3 || !node.nodeValue) return;
  const txt = node.nodeValue;
  _ERE.lastIndex = 0; if (!_ERE.test(txt)) return;
  _ERE.lastIndex = 0;
  const frag = document.createDocumentFragment();
  let last = 0, m;
  while ((m = _ERE.exec(txt))) {
    const base = m[0].replace(/\uFE0F/g, "");
    if (m.index > last) frag.appendChild(document.createTextNode(txt.slice(last, m.index)));
    const tmp = document.createElement('span'); tmp.innerHTML = _svg(EMAP[base] || '');
...
new MutationObserver(ms => ms.forEach(m => m.addedNodes.forEach(n => { _fn(n); _iconize(n); })))
  .observe(document.body, {childList: true, subtree: true});
_fn(document.body); _iconize(document.body); // fix initial DOM (sidebar nav links)
